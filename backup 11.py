...
def character_selection_screen():
    ...
    # 각 캐릭터의 크기와 위치 계산
    char_width = char1.get_width()
    char_height = char1.get_height()
    gap = 50  # 캐릭터 간 간격

    # 캐릭터의 x 위치 계산 (중앙에서 간격을 주어 수평 배치)
    char1_rect = char1.get_rect(midleft=(screen.get_width() // 2 - char_width - gap, screen.get_height() // 2))
    char2_rect = char2.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2))
    char3_rect = char3.get_rect(midright=(screen.get_width() // 2 + char_width + gap, screen.get_height() // 2))
...
def update_background():
    global background_x
    background_x -= background_speed  # 배경을 왼쪽으로 이동

    # 배경이 화면을 벗어나면 다시 초기 위치로 설정
    if background_x <= -800:  # 화면 크기 만큼 이동하면 초기화
        background_x = 0
...
# 배경 이동 변수
background_x = 0
background_speed = 2  # 배경이 이동하는 속도
...
while True:
    ...
    if not character_selected:
        ...
        # Game is running
        update_background()  # 배경 이동을 갱신
        screen.blit(backgrounds[current_background_index], (background_x, 0))  # 배경 1
        screen.blit(backgrounds[current_background_index], (background_x + 800, 0))  # 배경 2 (연속적으로 두 배경을 그려서 이동 효과를 줌)
        ...
        # 게임 시작 화면에서 캐릭터 중앙 배치
        # 게임 오버 화면에서 캐릭터 중앙 배치
        player_stand = pygame.image.load(f'graphics/Player/{character_choice}/stand.png').convert_alpha()
        player_stand = pygame.transform.rotozoom(player_stand, 0, 2)  # 크기를 2배로 키우기
